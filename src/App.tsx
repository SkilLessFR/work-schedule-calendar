...
const hours: Record<string, string> = { M: '06:00', A: '14:00', N: '22:00', MID: '09:00', H8: '8 hours', OFF: 'AD' };
...
export default function App() {
  ...
  const filteredEmployees = roster?.employees.filter((name) => name.toLowerCase().includes(query.toLowerCase())) ?? [];
  ...
  const calendarDays = monthDays(currentMonth, currentYear);
  ...
  return <main className={dark ? 'dark' : ''}><div className="min-h-screen bg-zinc-100 text-zinc-950 transition dark:bg-black dark:text-white">
    <section className="mx-auto max-w-3xl px-4 py-8 sm:py-12">
      ...
      <div ref={printRef} className="overflow-hidden rounded-[2rem] bg-white shadow-2xl dark:bg-zinc-900"><div className="grid grid-cols-7 bg-zinc-50 text-center text-sm font-bold text-zinc-500 dark:bg-zinc-800">{weekdays.map((day, i) => <div className="py-3" key={`${day}-${i}`}>{day}</div>)}</div><div className="grid grid-cols-7">{calendarDays.map((day) => { const dayIso = iso(day); const event = eventMap[dayIso]; const colors = event ? colorFor(event.shift) : fallbackColor; return <button key={dayIso} onClick={() => event && setSelectedEvent(event)} className={`min-h-24 border-t border-zinc-200 p-1 text-left transition hover:bg-zinc-100 dark:border-zinc-800 dark:hover:bg-zinc-800 ${day.getMonth() !== currentMonth ? 'opacity-25' : ''}`}><span className={`mx-auto mb-2 flex size-8 items-center justify-center rounded-full text-lg font-semibold ${dayIso === todayIso ? 'bg-white text-black shadow' : ''}`}>{day.getDate()}</span>{event && <span className={`block rounded-md border px-1 py-1 text-center text-sm font-bold ${colors.bg} ${colors.text} ${colors.border}`}>{event.shift}<br/><small>{hours[event.shift.toUpperCase()] ?? ''}</small></span>}</button>; })}</div></div></>}
    </section>
    {selectedEvent && <div className="fixed inset-0 z-10 flex items-end bg-black/40" onClick={() => setSelectedEvent(null)}><div onClick={(e) => e.stopPropagation()} className="w-full rounded-t-[2rem] bg-white p-6 shadow-2xl animate-in slide-in-from-bottom dark:bg-zinc-900"><div className="mx-auto mb-5 h-1 w-12 rounded-full bg-zinc-300"/><p className="text-zinc-500">{selectedEvent.date.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p><div className={`my-5 rounded-3xl border p-6 text-center text-4xl font-black ${colorFor(selectedEvent.shift).bg} ${colorFor(selectedEvent.shift).text} ${colorFor(selectedEvent.shift).border}`}>{selectedEvent.shift}</div><p><b>Hours:</b> {hours[selectedEvent.shift.toUpperCase()] ?? 'Not provided'}</p><p><b>Notes:</b> {selectedEvent.notes ?? 'No notes'}</p><button onClick={() => setSelectedEvent(null)} className="mt-6 w-full rounded-2xl bg-zinc-950 py-4 font-bold text-white dark:bg-white dark:text-black">Close</button></div></div>}
  </div></main>;
}
